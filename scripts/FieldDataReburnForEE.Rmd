...
# Assign unique row ideas, add file name
lcsites = read_csv("../data/fieldCsv/LC_SITES_11142023.csv")
lcdata = read_csv("../data/fieldCsv/LC_Data_09052023.csv")
fd = read_csv("../data/fieldCsv/FiSL_sitedata_BB2023.csv")
fp = read_csv("../data/fieldCsv/FINAL_PLOTS_01142024.csv")
...
( lcdata_id = lcdata %>% 
    ...
    mutate(id = str_c(Researcher,"_",rowId)) %>% 
    mutate(file="LC_Data_09052023"))
```
```{r}
...
( lcsites_id = lcsites %>% 
    mutate(rowId = row_number()) %>% 
    mutate(researcher = "walkerLC") %>% 
    mutate(id = str_c(researcher,"_",rowId)) %>% 
    mutate(file="LC_SITES_11142023"))
```
...
( fp_id = fp %>% 
    ...
    mutate(id = str_c(researcher,"_",rowId)) %>% 
    mutate(file="FINAL_PLOTS_01142024") )
```
...
```{r}
( fp1 = fp_id %>% 
    dplyr::select(id, unique_site_id.x, date.x, official_fire_scar, site, latitude, longitude, recent_burn, second_burn, triple_burn, file) %>%
...
```{r}
( lc1 = lcdata_id %>% 
    dplyr::select(id, Researcher, Site_Name, Sampling_Date, StudyArea, Year_of_Burn, `...7`, `...8`, Latitude, Longitude, file) %>%
...
```{r}
( lcsites1 = lcsites_id %>% 
    dplyr::select(id, unique_site_id, date, official_fire_scar, site, latitude, longitude, recent_burn, second_burn, triple_burn, file) %>%
...
```
```{r}
write.csv(df, "../outputs/field/combinedFieldData.csv", row.names = FALSE)
```
