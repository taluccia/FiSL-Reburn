---
title: "AnalysisMLCompMultimonial"
author: "Anna Talucci"
date: "2025-11-12"
...
( multi_data = allData1 %>% dplyr::select(-tx, -tx_num, -tx4,  -reburn, -reburn_num) %>% drop_na() %>% mutate(tx4_num = as.factor(tx4_num)) )
...
write.csv(best_models, '../outputs/MLselection/2025-11-12_MLSelectionMultinomial.csv', row.names=F)
```
...
```{r eval=FALSE, include=FALSE}
write.csv(xgb_best, '../outputs/MLselection/2025-11-12_XGBbestMultinomial_metrics.csv', row.names=F)
```
...
```{r}
saveRDS(final_xgb_fit, "../model/final_xgb_multinomial_model.rds")
