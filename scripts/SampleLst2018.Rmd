...
caLst =  read_csv("../data/eeSampledData/lst/sampleLstCan2018.csv") %>% dplyr::select(-.geo) 
cafire = read_csv("../data/eeSampledData/burnedx/sampleFireCA2018.csv") %>% dplyr::select(-.geo)
```
```{r}
akLst = read_csv("../data/eeSampledData/lst/sampleLstAK2018.csv") %>% dplyr::select(-.geo) 
...
( caJoined = caLst %>% 
  ...
  dplyr::select(ID:longitude, dnbr1986:CtDnbr, maxYr, medianYr, minYr, sumYr, max_2018, mean_2018, median_2018, min_2018) %>%
    filter(countYr <=3) %>%
...
# Write to csv
```{r}
write.csv( akCancombined, '../outputs/cleanedSamplePoints/2018LSTSampleWesternCanadaAlaska.cvs', row.names=F)
```
